...
Protocol version 6 uses one little-endian 32-bit length prefix followed by an
...
- Semantic events are reliable and ordered by one monotonically increasing sequence. The current tracer bullet emits Terminal Session exit/failure and every Control Lease transition; later resource and agent modules extend this same event class rather than sharing the coalescible visual path.
...
- The stable recovery endpoint first requests graceful shutdown and a durable-state flush. After a bounded grace period it may terminate its own Core process without running Rust destructors; acknowledgement means the stop was accepted, and the caller confirms endpoint disappearance before starting a replacement.
